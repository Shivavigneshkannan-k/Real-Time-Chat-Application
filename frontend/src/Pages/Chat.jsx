  ...
  return (
    <div className='flex justify-center items-center gap-4 w-full h-[calc(100vh-20vh)] relative'>
      <div className="flex items-center w-[calc(100%-1em)] m-2 gap-4 px-4 py-2 top-0 absolute bg-base-300 rounded-lg ">
        <img src={activeUser?.photoUrl || "../../public/landscape.avif"} alt={"user photo"} className="w-10 h-10 rounded-full object-cover  "/>
      ...
      <div className="w-full h-[80%] overflow-y-auto p-4">
        {chat &&
          chat.map((message) => (
            ...
            <div className={`${(message?.fromUserId===user?._id)?"chat-end":"chat-start"} w-full chat  `}>
              
              <div className='chat-bubble'>
                  {message?.photoUrl && <img src={message?.photoUrl} alt="loading failed" className="w-50 h-50 object-cover my-2"/>}
                  {message?.content && <div className="text-end px-2">{message?.content}</div>}
                </div>
            </div>
          ))}
      </div>
      ...
      <div className="flex items-center gap-4 absolute bottom-2 justify-center w-full">
        ...
        />
        <input className="input hidden image-input" type='file' ref={hiddenButtonRef} onChange={(e)=>{
          setNewMessage(prev =>({...prev,"image":e.target.files[0]}));
        }}title="image">
        </input>
        
        <button className='btn bg-primary' onClick={onSubmit}>send</button>
      </div>
      <div></div>
    </div>
  );
